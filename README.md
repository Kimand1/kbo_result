...
KBO 순위 또는 일별 순위 API 반영이 완료 경기 정보보다 늦으면, 스크립트는 수집한 완료 경기로 순위와 이력을 계산해 보완하고 안내 메시지를 출력합니다. 반대로 일정 페이지의 경기 리뷰 링크가 늦게 생성되면 게임센터의 종료 경기와 공식 팀별 경기 수가 정확히 일치할 때만 결과를 보완합니다. 최신 박스스코어가 아직 없으면 이전 완료 경기일까지 거슬러 올라가 불펜 정보를 구성합니다.
